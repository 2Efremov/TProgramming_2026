...
export abstract class kitsune {
  ...
  public attack(target: kitsune, logger: Logger): void {
    target.takeDamage(this._strength);
    logger.log(
      `(${this.className}) ${this.name} наносит урон ${this._strength} противнику (${target.className}) ${target.name}`,
    );
  }

  public abstract useAbility(target: kitsune, logger: Logger): void;

  public useIceArrows(target: kitsune, logger: Logger): boolean {
    if (this._iceArrowsUsed < this._iceArrowsLimit) {
      this._iceArrowsUsed++;
      target.receiveIceArrow(this._strength);
      logger.log(
        `(${this.className}) ${this.name} использует Ледяные стрелы против (${target.className}) ${target.name}!`,
      );
      return true;
    }
    return false;
  ...
  public applyEffects(logger: Logger): void {
    ...
    if (this._iceTurns > 0) {
      ...
      logger.log(
        `(${this.className}) ${this.name} получает ${iceDamage} доп. урона от обморожения. Осталось: ${this.health}`,
      );
      this._iceTurns--;
      if (this._iceTurns === 0) {
        this._iceStacks = 0;
